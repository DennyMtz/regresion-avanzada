...
wdir<-""
wdir<-"/home/denny/github/regresion-avanzada/codigo/m1-0"
setwd(wdir)
...
data<-list("n"=n,"y"=datos$homi_count,"exposure"=datos$POBTOT,"x1"= datos$POR_VPH_AUTOM,"x2"=datos$PROM_OCUP,"z"=unos)

#-Defining inits-
inits<-function(){list(beta=rep(1,3),gamma=rep(1,1),ypred=rep(1,n))}
...
#-Running code-
#OpenBUGS
zip_1_0.sim<-bugs(data,inits,parameters,model.file="ZIPoisson_1-0.txt",
                                 n.iter=50000,n.chains=1,n.burnin=5000)

out_zip_1_0.sim<-zip_1_0.sim$sims.list

traceplot(zip_1_0.sim)
plot(zip_1_0.sim)


#Resumen (estimadores)
#OpenBUGS
out.sum_zip_1_0.sim <- zip_1_0.sim$summary
#print(out.sum_zip_1_0.sim)
head(out.sum_zip_1_0.sim)
write.csv(out.sum_zip_1_0.sim,"out.sum_zip_1_0.sim.csv")
#DIC
zip_1_0.sim.dic<-zip_1_0.sim$DIC
print(zip_1_0.sim.dic) #8.4e+11

out.ypred<-out.sum_zip_1_0.sim [grep("ypred",rownames(out.sum_zip_1_0.sim )),]
cor(datos$homi_count,out.ypred[,1])
#0.6133022

######################## Analisis MCMC para beta1
beta_num <- 1
z<-out_zip_1_0.sim$beta[,beta_num]
par(mfrow=c(2,2))
plot(z,type="l")
...
######################## Analisis MCMC para gamma
z<-out_zip_1_0.sim$gamma
par(mfrow=c(2,2))
plot(z,type="l")
plot(cumsum(z)/(1:length(z)),type="l")
hist(z,freq=FALSE)
...
############################3
#Predictions
out.ypred<-out.ypred<-out.sum_zip_1_0.sim [grep("ypred",rownames(out.sum_zip_1_0.sim )),]
