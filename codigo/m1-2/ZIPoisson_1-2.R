...
wdir<-""
wdir<-"/home/denny/github/regresion-avanzada/codigo/m1-2"
setwd(wdir)
...
data<-list("n"=n,"y"=datos$homi_count,"exposure"=datos$POBTOT,"x"=datos$PROM_OCUP,"z"=datos$POR_VPH_AUTOM)
...
#-Running code-
#OpenBUGS
zip_1_2.sim <-bugs(data,inits,parameters,model.file="ZIPoisson_1-2.txt",
                  n.iter=20000,n.chains=1,n.burnin=5000)


out_zip_1_2.sim<-zip_1_2.sim$sims.list

traceplot(zip_1_2.sim)
plot(zip_1_2.sim)


#Resumen (estimadores)
#OpenBUGS
out.sum_zip_1_2.sim <- zip_1_2.sim$summary
#print(out.sum_zip_1_2.sim)
head(out.sum_zip_1_2.sim)
write.csv(out.sum_zip_1_2.sim,"out.sum_zip_1_2.sim.csv")


#DIC
zip_1_2.sim.dic<-zip_1_2.sim$DIC
print(zip_1_2.sim.dic) #8.4e+11

out.ypred<-out.sum_zip_1_2.sim [grep("ypred",rownames(out.sum_zip_1_2.sim )),]
cor(datos$homi_count,out.ypred[,1])
#0.5902142

######################## Analisis MCMC para beta1
beta_num <- 1
z<-out_zip_1_2.sim$beta[,beta_num]
par(mfrow=c(2,2))
plot(z,type="l")
...
gamma_num <- 1
z<-out_zip_1_2.sim$gamma[,gamma_num]
par(mfrow=c(2,2))
plot(z,type="l")
plot(cumsum(z)/(1:length(z)),type="l")
hist(z,freq=FALSE)
...
############################3
#Predictions
out.ypred<-out.ypred<-out.sum_zip_1_2.sim [grep("ypred",rownames(out.sum_zip_1_2.sim )),]
